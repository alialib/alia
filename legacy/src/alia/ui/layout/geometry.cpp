...
scoped_clip_region::set(box<2, double> const& region)
{
    ...
    // if (!compute_intersection(&new_region, old_region_,
    // region_in_root_frame))
    //     new_region = box<2, double>(make_vector(0., 0.), make_vector(0.,
    //     0.));
    // set_clip_region(*ctx_, new_region);
    set_clip_region(*ctx_, region);
}
void
scoped_clip_region::end()
{
    ...
    {
        if (ctx_->subscriber)
            ctx_->subscriber->pop_state();
        ctx_->clip_region = old_region_;
        ctx_ = 0;
    }
}
...
scoped_transformation::end()
{
    if (ctx_)
    {
        ctx_->transformation_matrix = old_matrix_;
        if (ctx_->subscriber)
            ctx_->subscriber->pop_state();
        ctx_ = 0;
    }
