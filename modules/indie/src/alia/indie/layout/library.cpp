    ...
    layout_scalar assigned_baseline_y)
{
    assign_identical_child_regions(
        ctx, children, assigned_size, assigned_baseline_y);
}
    ...
    layout_scalar assigned_width)
{
    ...
    for (layout_node* i = children; i; i = i->next)
    {
        ...
        while (!layout_scalars_almost_equal(upper_bound, lower_bound))
        {
            ...
            layout_requirements y
                = alia::get_vertical_requirements(ctx, *i, test_value);
    ...
    layout_calculation_context& ctx, layout_node* children)
{
    return calculated_layout_requirements(
        get_max_child_width(ctx, children) + (border.left + border.right),
        0,
        0);
}
    ...
    layout_scalar assigned_width)
{
    calculated_layout_requirements requirements
        = fold_vertical_child_requirements(
            ctx, children, assigned_width - (border.left + border.right));
    return calculated_layout_requirements(
...
template<class Uniformity>
void
update_grid_column_requirements(
    layout_calculation_context& ctx, grid_data<Uniformity>& grid)
{
    named_block nb;
    ALIA_BEGIN_LOCATION_SPECIFIC_NAMED_BLOCK(ctx.naming, nb, make_id(grid.id));
    ...
    alia_if(grid.last_content_query != grid.container->last_content_change)
    {
        ...
        for (grid_row_container<Uniformity>* row = grid.rows; row;
             row = row->next)
        {
            named_block row_block(nc, make_id(row));
    ...
    layout_calculation_context& ctx)
{
    horizontal_layout_query query(
        ctx, cacher, grid->container->last_content_change);
    ...
    layout_calculation_context& ctx, layout_scalar assigned_width)
{
    vertical_layout_query query(
        ctx, cacher, grid->container->last_content_change, assigned_width);
    alia_if(query.update_required())
    {
    ...
    relative_layout_assignment const& assignment)
{
    relative_region_assignment rra(
        ctx, *this, cacher, grid->container->last_content_change, assignment);
...
floating_layout::end()
{
    if (traversal_)
    {
        ...
        if (traversal.is_refresh_pass)
        {
            ...
            resolve_layout(
                floating_root_, data_->placement_cache, data_->size);
        }
