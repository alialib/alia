...
class ascii_text_renderer : boost::noncopyable
{
 public:
    void initialize(surface& surface, font const& font,
        ...
        int const* trailing_space,
        font_metrics const& metrics);

    font get_font() const { return font_; }

    // A character's width includes its trailing space, so this is the full
    // increment between two characters.
    int get_char_width(char c) const;

    int get_trailing_space(char c) const;
 ...
 private:
    ...
    int const* widths_;
    // trailing_space_ stores the amount of white space after each character
    int const* trailing_space_;
