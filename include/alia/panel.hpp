...
class panel_background : boost::noncopyable
{
 public:
    ...
    panel_background(context& ctx, unsigned style_code, unsigned flags,
        layout const& layout_spec, region_id id)
    { begin(ctx, style_code, flags, layout_spec, id); }
    ~panel_background() { end(); }
    void begin(context& ctx, unsigned style_code, unsigned flags,
        layout const& layout_spec, region_id id);
    void end();
...
class panel : boost::noncopyable
{
 public:
    panel() {}
    panel(context& ctx, style style, unsigned flags = 0,
        layout const& layout_spec = default_layout, region_id id = auto_id)
    { begin(ctx, style, flags, layout_spec, id); }
    panel(context& ctx, unsigned style_code, unsigned flags = 0,
        layout const& layout_spec = default_layout, region_id id = auto_id)
    { begin(ctx, style_code, flags, layout_spec, id); }
    ~panel() { end(); }
    void begin(context& ctx, style style, unsigned flags = 0,
        layout const& layout_spec = default_layout, region_id id = auto_id);
    void begin(context& ctx, unsigned style_code, unsigned flags = 0,
        layout const& layout_spec = default_layout, region_id id = auto_id);
    void end();
    bool is_relevant() const;
...
class scrollable_panel : boost::noncopyable
{
 public:
    scrollable_panel() {}
    scrollable_panel(context& ctx, style style, unsigned flags = 0,
        layout const& layout_spec = default_layout, region_id id = auto_id)
    { begin(ctx, style, flags, layout_spec, id); }
    scrollable_panel(context& ctx, unsigned style_code, unsigned flags = 0,
        layout const& layout_spec = default_layout, region_id id = auto_id)
    { begin(ctx, style_code, flags, layout_spec, id); }
