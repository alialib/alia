...
struct layout_flags
{
    unsigned code;
    layout_flags() {}
    explicit layout_flags(unsigned code) : code(code) {}
    // allows use within if statements without other unintended conversions
    typedef unsigned layout_flags::* unspecified_bool_type;
    operator unspecified_bool_type() const
    { return code != 0 ? &layout_flags::code : 0; }
};
static inline layout_flags operator|(layout_flags a, layout_flags b)
{ return layout_flags(a.code | b.code); }
static inline layout_flags& operator|=(layout_flags& a, layout_flags b)
...
static inline layout_flags& operator&=(layout_flags& a, layout_flags b)
{ a.code &= b.code; return a; }
static inline bool operator==(layout_flags a, layout_flags b)
{ return a.code == b.code; }

static layout_flags const no_layout_flags(0x000);

// X alignment flags
static unsigned const CENTER_X_CODE                                 = 0x001;
static layout_flags const CENTER_X(CENTER_X_CODE);
static unsigned const LEFT_CODE                                     = 0x002;
static layout_flags const LEFT(LEFT_CODE);
static unsigned const RIGHT_CODE                                    = 0x003;
static layout_flags const RIGHT(RIGHT_CODE);
static unsigned const FILL_X_CODE                                   = 0x004;
static layout_flags const FILL_X(FILL_X_CODE);
// GROW is the same as FILL, but it also sets the proportion to 1 if you don't
// manually override it.
static unsigned const GROW_X_CODE                                   = 0x00c;
static layout_flags const GROW_X(GROW_X_CODE);
static unsigned const X_ALIGNMENT_MASK_CODE                         = 0x00f;
static layout_flags const X_ALIGNMENT_MASK(X_ALIGNMENT_MASK_CODE);

// Y alignment flags
static unsigned const CENTER_Y_CODE                                 = 0x010;
static layout_flags const CENTER_Y(CENTER_Y_CODE);
static unsigned const TOP_CODE                                      = 0x020;
static layout_flags const TOP(TOP_CODE);
static unsigned const BOTTOM_CODE                                   = 0x030;
static layout_flags const BOTTOM(BOTTOM_CODE);
static unsigned const FILL_Y_CODE                                   = 0x040;
static layout_flags const FILL_Y(FILL_Y_CODE);
static unsigned const BASELINE_Y_CODE                               = 0x050;
static layout_flags const BASELINE_Y(BASELINE_Y_CODE);
static unsigned const GROW_Y_CODE                                   = 0x0c0;
static layout_flags const GROW_Y(GROW_Y_CODE);
static unsigned const Y_ALIGNMENT_MASK_CODE                         = 0x0f0;
static layout_flags const Y_ALIGNMENT_MASK(Y_ALIGNMENT_MASK_CODE);

// combined alignment flags
static layout_flags const CENTER(CENTER_X_CODE | CENTER_Y_CODE);
static layout_flags const FILL(FILL_X_CODE | FILL_Y_CODE);
static layout_flags const GROW(GROW_X_CODE | GROW_Y_CODE);
...
static layout_flags const PROPORTIONAL_FILL(PROPORTIONAL_FILL_CODE);
static unsigned const PROPORTIONAL_GROW_CODE                        = 0x0ff;
static layout_flags const PROPORTIONAL_GROW(PROPORTIONAL_GROW_CODE);
static unsigned const ALIGNMENT_MASK_CODE                           = 0x0ff;
static layout_flags const ALIGNMENT_MASK(ALIGNMENT_MASK_CODE);

// other flags
// explicitly enabled or disable the padding around a widget
static unsigned const PADDED_CODE                                   = 0x100;
static layout_flags const PADDED(PADDED_CODE);
static unsigned const NOT_PADDED_CODE                               = 0x200;
static layout_flags const NOT_PADDED(NOT_PADDED_CODE);
...
struct layout
{
    layout(alia::size const& size = alia::size(-1, -1, PIXELS),
        layout_flags flags = no_layout_flags, float proportion = 0)
      : size(size)
      , flags(flags)
      , proportion(proportion)
    {}
    layout(layout_flags flags, float proportion = 0)
      : size(-1, -1, PIXELS)
      , flags(flags)
      , proportion(proportion)
    {}
