...
struct alia_gl_shell
{
    alia_ui_controller inner{};
    alia_gl_shell_config config{};
...
{
    auto* shell = static_cast<alia_gl_shell*>(user_data);
    ALIA_ASSERT(shell);
...
alia_gl_shell*
alia_gl_shell_create(void)
{
    return new alia_gl_shell{};
}

void
alia_gl_shell_destroy(alia_gl_shell* shell)
{
    ALIA_ASSERT(shell);
...
alia_ui_controller
alia_gl_shell_ui_controller(alia_gl_shell* shell)
{
    ALIA_ASSERT(shell);
...
alia_gl_shell_text_engine(alia_gl_shell* shell)
{
    ALIA_ASSERT(shell);
    ...
    size_t font_count)
{
    ...
    alia_gl_shell_teardown_text(shell, ui);
    ...
    alia_gl_renderer_upload_msdf_atlas(renderer, &atlas_image);
...
void
alia_gl_shell_teardown_text(alia_gl_shell* shell, alia_ui_system* ui)
{
    ALIA_ASSERT(shell);
    ALIA_ASSERT(ui);
...
void
alia_gl_shell_draw(alia_ui_system* ui)
{
    ALIA_ASSERT(ui);
...
void
alia_gl_shell_frame(alia_ui_system* ui)
{
    ALIA_ASSERT(ui);
    alia_ui_system_update(ui);
    alia_gl_shell_draw(ui);
}

void
alia_gl_shell_initial_refresh(alia_ui_system* ui)
{
    ALIA_ASSERT(ui);
    refresh_system(*ui);
