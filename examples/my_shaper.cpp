...
using HBBuffer
    = std::unique_ptr<hb_buffer_t, SkFunctionObject<hb_buffer_destroy>>;

using SkUnicodeBreak = std::unique_ptr<SkBreakIterator>;

hb_position_t
skhb_position(SkScalar value)
{
    ...
    void* user_data)
{
    ...
    for (unsigned i = 0; i < count; i++)
    {
        unicode[i] = *unicodes;
        unicodes
            = SkTAddOffset<const hb_codepoint_t>(unicodes, unicode_stride);
    }
    AutoSTMalloc<256, SkGlyphID> glyph(count);
    font.textToGlyphs(
        unicode.get(),
        count * sizeof(SkUnichar),
    ...
    // return 'done' to allow HarfBuzz to synthesize with NFC and spaces,
    // return 'count' to avoid
    return done;
}

hb_position_t
skhb_glyph_h_advance(
    hb_font_t* hb_font,
    void* font_data,
...
skhb_get_font_funcs()
{
    static hb_font_funcs_t* const funcs = [] {
        // HarfBuzz will use the default (parent) implementation if they aren't
        // set.
        hb_font_funcs_t* const funcs = hb_font_funcs_create();
        hb_font_funcs_set_variation_glyph_func(
            funcs, skhb_glyph, nullptr, nullptr);
        hb_font_funcs_set_nominal_glyph_func(
            funcs, skhb_nominal_glyph, nullptr, nullptr);
#if SK_HB_VERSION_CHECK(2, 0, 0)
        hb_font_funcs_set_nominal_glyphs_func(
            funcs, skhb_nominal_glyphs, nullptr, nullptr);
#else
        sk_ignore_unused_variable(skhb_nominal_glyphs);
#endif
        hb_font_funcs_set_glyph_h_advance_func(
            funcs, skhb_glyph_h_advance, nullptr, nullptr);
#if SK_HB_VERSION_CHECK(1, 8, 6)
        hb_font_funcs_set_glyph_h_advances_func(
            funcs, skhb_glyph_h_advances, nullptr, nullptr);
#else
        sk_ignore_unused_variable(skhb_glyph_h_advances);
#endif
        hb_font_funcs_set_glyph_extents_func(
            funcs, skhb_glyph_extents, nullptr, nullptr);
...
hb_blob_t*
skhb_get_table(hb_face_t* face, hb_tag_t tag, void* user_data)
{
    ...
    return hb_blob_create(
        reinterpret_cast<char*>(rawData->writable_data()),
        rawData->size(),
...
HBBlob
stream_to_blob(std::unique_ptr<SkStreamAsset> asset)
{
    ...
    HBFace create_hb_face(const SkTypeface& typeface)
{
    ...
    std::unique_ptr<SkStreamAsset> typefaceAsset
        = typeface.openExistingStream(&index);
    HBFace face;
    if (typefaceAsset && typefaceAsset->getMemoryBase())
    {
        ...
        // recognized first. hb_face_create_for_tables may still create a
        // working hb_face. See https://github.com/harfbuzz/harfbuzz/issues/248
        // .
...
HBFont
create_typeface_hb_font(const SkTypeface& typeface)
{
    ...
    if (axis_count > 0)
    {
        AutoSTMalloc<4, SkFontArguments::VariationPosition::Coordinate>
            axis_values(axis_count);
        if (typeface.getVariationDesignPosition(axis_values, axis_count)
            == axis_count)
        {
...
HBFont
create_sub_hb_font(const SkFont& font, const HBFont& typefaceFont)
{
...
static inline SkUnichar
utf8_next(const char** ptr, const char* end)
{
    SkUnichar val = SkUTF::NextUTF8(ptr, end);
    return val < 0 ? 0xFFFD : val;
}

class SkUnicodeHbScriptRunIterator final : public MyShaper::ScriptRunIterator
{
 public:
    SkUnicodeHbScriptRunIterator(
        const char* utf8, size_t utf8Bytes, hb_script_t defaultScript)
        : fCurrent(utf8),
          fBegin(utf8),
    ...
    hb_script_t
    hb_script_for_unichar(SkUnichar u)
    {
    ...
    size_t
    endOfCurrentRun() const override
    {
...
class RunIteratorQueue
{
 public:
    void
    insert(MyShaper::RunIterator* runIterator, int priority)
    {
    ...
    advanceRuns()
    {
        const MyShaper::RunIterator* leastRun = fEntries.peek().runIterator;
        if (leastRun->atEnd())
        {
        ...
        MyShaper::RunIterator* currentRun = nullptr;
        SkDEBUGCODE(size_t previousEndOfCurrentRun);
        while ((currentRun = fEntries.peek().runIterator)->endOfCurrentRun()
               <= leastEnd)
        {
 ...
 private:
    ...
    struct Entry
    {
        MyShaper::RunIterator* runIterator;
        int priority;
    };
    static bool
    CompareEntry(Entry const& a, Entry const& b)
    {
...
{
    ShapedRun(
        MyShaper::RunHandler::Range utf8Range,
        const SkFont& font,
    ...
    MyShaper::RunHandler::Range fUtf8Range;
    SkFont fFont;
...
constexpr bool
is_LTR(SkBidiIterator::Level level)
{
    return (level & 1) == 0;
}
...
void
append(
    MyShaper::RunHandler* handler,
    const MyShaper::RunHandler::RunInfo& runInfo,
    const ShapedRun& run,
    ...
    size_t endGlyphIndex)
{
    ...
    {
        // Glyphs are in logical order, but output ltr since PDF readers seem
        // to expect that.
        const ShapedGlyph& glyph
            = run.fGlyphs
                  [is_LTR(run.fLevel) ? startGlyphIndex + i
                                      : endGlyphIndex - 1 - i];
...
void
emit(SkUnicode* unicode, const ShapedLine& line, MyShaper::RunHandler* handler)
{
    ...
    for (int i = 0; i < numRuns; ++i)
    {
        ...
        const MyShaper::RunHandler::RunInfo info
            = {run.fFont,
               run.fLevel,
               run.fAdvance,
               run.fNumGlyphs,
...
struct ShapedRunGlyphIterator
{
    ...
    operator=(const ShapedRunGlyphIterator& that)
        = default;
    bool
    operator==(const ShapedRunGlyphIterator& that) const
    {
...
class ShaperHarfBuzz : public MyShaper
{
 public:
    ShaperHarfBuzz(std::unique_ptr<SkUnicode>, HBBuffer, sk_sp<SkFontMgr>);
...
static std::unique_ptr<MyShaper>
MakeHarfBuzz(sk_sp<SkFontMgr> fallback, bool correct)
{
    ...
    RunHandler* handler) const
{
    ...
    this->shape(
        utf8, utf8Bytes, *font, *bidi, *script, *language, width, handler);
}
    ...
    RunHandler* handler) const
{
    ...
    RunIteratorQueue runSegmenter;
    runSegmenter.insert(
        &font, 3); // The font iterator is always run last in case of tie.
    ...
    RunHandler* handler) const
{
    ...
    { // For each item
        ...
        // map from character position to [safe to break, glyph position,
        // advance]
        ...
        { // While there are still code points left in this item
            ...
            // TODO: break iterator per item, but just reset position if
            // needed? Maybe break iterator with model?
            if (!lineBreakIterator
                || !currentLanguage.equals(language.currentLanguage()))
            {
                currentLanguage = language.currentLanguage();
                lineBreakIterator = fUnicode->makeBreakIterator(
                    currentLanguage.c_str(), SkUnicode::BreakType::kLines);
                if (!lineBreakIterator)
                {
                    return;
                }
            ...
            {
                // TODO: if past a safe to break, future safe to break will be
                // at least as long
                ...
                ShapedRun candidate = [&](const TextProps& props) {
                    if (props.glyphLen)
                    {
                        ...
                        return ShapedRun(
                            RunHandler::Range(
                                utf8Start - utf8, breakIteratorCurrent),
                            font.currentFont(),
                            bidi.currentLevel(),
            ...
            // If nothing fit (best score is negative) and the line is not
            // empty
            ...
            else
            {
                ...
                {
                    best.fGlyphs
                        = std::make_unique<ShapedGlyph[]>(best.fNumGlyphs);
                    memcpy(
                        best.fGlyphs.get(),
                        model.fGlyphs.get() + modelGlyphOffset,
                ...
                // If item broken, emit line (prevent remainder from
                // accidentally fitting)
    ...
    RunHandler* handler) const
{
    TArray<ShapedRun> runs;
    {
        ...
        while (runSegmenter.advanceRuns())
        {
                ...
                || !currentLanguage.equals(language.currentLanguage()))
            {
                ...
                graphemeBreakIterator = fUnicode->makeBreakIterator(
                    currentLanguage.c_str(), SkUnicode::BreakType::kGraphemes);
            ...
            for (size_t i = 0; i < run.fNumGlyphs; ++i)
            {
                ...
                int32_t lineBreakIteratorCurrent
                    = lineBreakIterator->current();
                while (!lineBreakIterator->isDone()
                       && lineBreakIteratorCurrent < glyphCluster)
                {
                    lineBreakIteratorCurrent = lineBreakIterator->next();
                }
                glyph.fMayLineBreakBefore
                    = glyph.fCluster != previousCluster
                      && lineBreakIteratorCurrent == glyphCluster;

                int32_t graphemeBreakIteratorCurrent
                    = graphemeBreakIterator->current();
                while (!graphemeBreakIterator->isDone()
                       && graphemeBreakIteratorCurrent < glyphCluster)
                {
                    graphemeBreakIteratorCurrent
    ...
    // Iterate over the glyphs in logical order to find potential line lengths.
    {
        ...
        {
            // 'Break' at graphemes until a line boundary, then only at line
            // boundaries. Only break at graphemes if no line boundary is
            // valid.
            ...
            // Break when overwidth, the glyph has a visual representation, and
            // some space is used.
    ...
    // Reorder the runs and glyphs per line and write them out.
    {
        ...
        while (glyphIterator.current())
        {
            ...
            // step through the runs in reverse visual order and the glyphs in
            // reverse logical order until a visible glyph is found and force
            // them to the end of the visual line.
            ...
            auto makeSubRun = [&runs,
                               &previousBreak,
                               &current,
                               &logicalFromVisual](size_t visualIndex) {
                int logicalIndex
                    = previousBreak.fRunIndex + logicalFromVisual[visualIndex];
            ...
            auto makeRunInfo = [](const SubRun& sub) {
                uint32_t startUtf8
                    = sub.run.fGlyphs[sub.startGlyphIndex].fCluster;
                uint32_t endUtf8
                    = (sub.endGlyphIndex < sub.run.fNumGlyphs)
                          ? sub.run.fGlyphs[sub.endGlyphIndex].fCluster
                          : sub.run.fUtf8Range.end();

                SkVector advance = SkVector::Make(0, 0);
                for (size_t i = sub.startGlyphIndex; i < sub.endGlyphIndex;
                     ++i)
                {
...
class HBLockedFaceCache
{
 public:
    HBLockedFaceCache(
        SkLRUCache<SkTypefaceID, HBFont>& lruCache, SkMutex& mutex)
    ...
    HBLockedFaceCache&
    operator=(const HBLockedFaceCache&)
        = delete;
    HBLockedFaceCache&
    operator=(HBLockedFaceCache&&)
        = delete;
    ...
    HBFont*
    find(SkTypefaceID fontId)
    {
        return fLRUCache.find(fontId);
    }
    HBFont*
    insert(SkTypefaceID fontId, HBFont hbFont)
    {
    ...
    size_t const featuresSize) const
{
    size_t utf8runLength = utf8End - utf8Start;
    ShapedRun run(
        RunHandler::Range(utf8Start - utf8, utf8runLength),
        font.currentFont(),
        bidi.currentLevel(),
        nullptr,
        0);
    ...
    // dotted circle, re-evaluate if BOT/EOT change. See
    // https://skbug.com/9618. hb_buffer_set_flags(buffer, HB_BUFFER_FLAG_BOT |
    // HB_BUFFER_FLAG_EOT);
    ...
    hb_buffer_set_script(
        buffer,
        hb_script_from_iso15924_tag((hb_tag_t) script.currentScript()));
    // Buffers with HB_LANGUAGE_INVALID race since hb_language_get_default is
    // not thread safe. The user must provide a language, but may provide data
    // hb_language_from_string cannot use. Use "und" for the undefined language
    // in this case (RFC5646 4.1 5).
    ...
    HBFont hbFont;
    {
        ...
        {
            HBFont typefaceFont(
                create_typeface_hb_font(*font.currentFont().getTypeface()));
    ...
    for (unsigned i = 0; i < len; i++)
    {
        ...
        glyph.fHasVisual
            = !glyphBounds[i]
                   .isEmpty(); //! font->currentTypeface()->glyphBoundsAreZero(glyph.fID);
...
std::unique_ptr<MyShaper::ScriptRunIterator>
MyShaper::MakeHbIcuScriptRunIterator(const char* utf8, size_t utf8Bytes)
{
    return MyShaper::MakeSkUnicodeHbScriptRunIterator(utf8, utf8Bytes);
}

std::unique_ptr<MyShaper::ScriptRunIterator>
MyShaper::MakeSkUnicodeHbScriptRunIterator(const char* utf8, size_t utf8Bytes)
{
...
std::unique_ptr<MyShaper::ScriptRunIterator>
MyShaper::MakeSkUnicodeHbScriptRunIterator(
...
std::unique_ptr<MyShaper>
MyShaper::MakeShaperDrivenWrapper(sk_sp<SkFontMgr> fontmgr)
{
    return MakeHarfBuzz(std::move(fontmgr), true);
}
std::unique_ptr<MyShaper>
MyShaper::MakeShapeThenWrap(sk_sp<SkFontMgr> fontmgr)
{
    return MakeHarfBuzz(std::move(fontmgr), false);
}
std::unique_ptr<MyShaper>
MyShaper::MakeShapeDontWrapOrReorder(
    std::unique_ptr<SkUnicode> unicode, sk_sp<SkFontMgr> fontmgr)
{
    HBBuffer buffer(hb_buffer_create());
    if (!buffer)
    {
...
std::unique_ptr<MyShaper>
MyShaper::Make(sk_sp<SkFontMgr> fallback)
{
#ifdef SK_SHAPER_HARFBUZZ_AVAILABLE
    std::unique_ptr<MyShaper> shaper
        = MyShaper::MakeShaperDrivenWrapper(std::move(fallback));
    if (shaper)
    {
        return shaper;
    }
#elif defined(SK_SHAPER_CORETEXT_AVAILABLE)
    if (auto shaper = MyShaper::MakeCoreText())
    {
        return shaper;
    }
#endif
#if defined(SK_SHAPER_PRIMITIVE_AVAILABLE)
    return MyShaper::MakePrimitive();
#else
    return nullptr;
#endif
}
...
std::unique_ptr<MyShaper::BiDiRunIterator>
MyShaper::MakeBiDiRunIterator(
    const char* utf8, size_t utf8Bytes, uint8_t bidiLevel)
{
#ifdef SK_SHAPER_UNICODE_AVAILABLE
    if (const auto unicode = SkUnicode::Make())
    {
        std::unique_ptr<MyShaper::BiDiRunIterator> bidi
            = MyShaper::MakeSkUnicodeBidiRunIterator(
                unicode.get(), utf8, utf8Bytes, bidiLevel);
        if (bidi)
        {
            return bidi;
        }
...
#endif
    return std::make_unique<MyShaper::TrivialBiDiRunIterator>(
        bidiLevel, utf8Bytes);
}

std::unique_ptr<MyShaper::ScriptRunIterator>
MyShaper::MakeScriptRunIterator(
    const char* utf8, size_t utf8Bytes, SkFourByteTag scriptTag)
{
#if defined(SK_SHAPER_HARFBUZZ_AVAILABLE)
    std::unique_ptr<MyShaper::ScriptRunIterator> script
        = MyShaper::MakeSkUnicodeHbScriptRunIterator(
            utf8, utf8Bytes, scriptTag);
    if (script)
    {
        return script;
    }
#endif
    return std::make_unique<MyShaper::TrivialScriptRunIterator>(
        scriptTag, utf8Bytes);
}
...
class FontMgrRunIterator final : public MyShaper::FontRunIterator
{
 public:
    FontMgrRunIterator(
        const char* utf8,
        ...
        const MyShaper::LanguageRunIterator* lang)
        : fCurrent(utf8),
          fBegin(utf8),
          fEnd(fCurrent + utf8Bytes),
          ...
          fLanguage(lang)
    {
        // If fallback is not wanted, clients should use
    ...
    consume() override
    {
        ...
        if (fFont.unicharToGlyph(u))
        {
            ...
            fFallbackFont.getTypeface() && fFallbackFont.unicharToGlyph(u))
        {
        ...
        else
        {
            ...
            sk_sp<SkTypeface> candidate(
                fFallbackMgr->matchFamilyStyleCharacter(
        ...
        while (fCurrent < fEnd)
        {
            ...
            // End run if not using initial typeface and initial typeface has
            // this character.
            ...
            // End run if current typeface does not have this character and
            // some other font does.
 ...
 private:
    ...
    MyShaper::LanguageRunIterator const* const fLanguage;
};

std::unique_ptr<MyShaper::FontRunIterator>
MyShaper::MakeFontMgrRunIterator(
    const char* utf8,
    size_t utf8Bytes,
    const SkFont& font,
    sk_sp<SkFontMgr> fallback)
{
    return std::make_unique<FontMgrRunIterator>(
        utf8, utf8Bytes, font, std::move(fallback));
}
...
MyShaper::MakeFontMgrRunIterator(
    ...
    const char* requestName,
    SkFontStyle requestStyle,
    const MyShaper::LanguageRunIterator* language)
{
    return std::make_unique<FontMgrRunIterator>(
        utf8,
        utf8Bytes,
...
std::unique_ptr<MyShaper::LanguageRunIterator>
MyShaper::MakeStdLanguageRunIterator(const char* utf8, size_t utf8Bytes)
{
...
void
MySkTextBlobBuilderRunHandler::beginLine()
{
...
void
MySkTextBlobBuilderRunHandler::runInfo(const RunInfo& info)
{
    SkFontMetrics metrics;
...
void
MySkTextBlobBuilderRunHandler::commitRunInfo()
{
    fCurrentPosition.fY -= fMaxRunAscent;
}

MyShaper::RunHandler::Buffer
MySkTextBlobBuilderRunHandler::runBuffer(const RunInfo& info)
{
    int glyphCount
...
void
MySkTextBlobBuilderRunHandler::commitRunBuffer(const RunInfo& info)
{
...
void
MySkTextBlobBuilderRunHandler::commitLine()
{
    fOffset += {0, fMaxRunDescent + fMaxRunLeading - fMaxRunAscent};
}

sk_sp<SkTextBlob>
MySkTextBlobBuilderRunHandler::makeBlob()
{
...
class SkUnicodeBidiRunIterator final : public MyShaper::BiDiRunIterator
{
 public:
    SkUnicodeBidiRunIterator(
        const char* utf8, const char* end, SkUnicodeBidi bidi)
        : fBidi(std::move(bidi)),
...
{
    // ubidi only accepts utf16 (though internally it basically works on utf32
    // chars). We want an ubidi_setPara(UBiDi*, UText*, UBiDiLevel,
    ...
    SkUnicodeBidi bidi
        = unicode->makeBidiIterator(utf16.get(), utf16Units, bidiDir);
    if (!bidi)
    {
        SkDEBUGF("Bidi error\n");
        return nullptr;
    ...
    return std::make_unique<SkUnicodeBidiRunIterator>(
        utf8, utf8 + utf8Bytes, std::move(bidi));
}

std::unique_ptr<MyShaper::BiDiRunIterator>
MyShaper::MakeIcuBiDiRunIterator(
    const char* utf8, size_t utf8Bytes, uint8_t bidiLevel)
{
    auto unicode = SkUnicode::Make();
    if (!unicode)
    ...
    return MyShaper::MakeSkUnicodeBidiRunIterator(
        unicode.get(), utf8, utf8Bytes, bidiLevel);
}
