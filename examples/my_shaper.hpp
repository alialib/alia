...
#ifndef MY_MyShaper_DEFINED
#define MY_MyShaper_DEFINED
...
class SKSHAPER_API MyShaper
{
 public:
    ...
    static std::unique_ptr<MyShaper>
    MakeShaperDrivenWrapper(sk_sp<SkFontMgr> fallback);
    static std::unique_ptr<MyShaper>
    MakeShapeThenWrap(sk_sp<SkFontMgr> fallback);
    static std::unique_ptr<MyShaper>
    MakeShapeDontWrapOrReorder(
        std::unique_ptr<SkUnicode> unicode, sk_sp<SkFontMgr> fallback);
    static void
    PurgeHarfBuzzCache();
#endif
    ...
    static std::unique_ptr<MyShaper>
    Make(sk_sp<SkFontMgr> fallback = nullptr);
    static void
    PurgeCaches();
    ...
    class RunIterator
    {
     public:
        ...
        /** Set state to that of current run and move iterator to end of that
         * run. */
        virtual void
        consume()
            = 0;
 ...
 private:
    ...
    class TrivialRunIterator : public RunIteratorSubclass
    {
     public:
        static_assert(
            std::is_base_of<RunIterator, RunIteratorSubclass>::value, "");
        TrivialRunIterator(size_t utf8Bytes)
 ...
 public:
    ...
    MakeFontMgrRunIterator(
        const char* utf8,
        size_t utf8Bytes,
        const SkFont& font,
        sk_sp<SkFontMgr> fallback);
    static std::unique_ptr<MyShaper::FontRunIterator>
    MakeFontMgrRunIterator(
        const char* utf8,
        size_t utf8Bytes,
        const SkFont& font,
        ...
        const MyShaper::LanguageRunIterator*);
        ...
        : public TrivialRunIterator<LanguageRunIterator>
    {
     public:
        ...
        const char*
        currentLanguage() const override
        {
    ...
    class RunHandler
    {
     public:
        ...
        struct Buffer
        {
            ...
            SkPoint* positions; // required, if (!offsets) put glyphs[i] at
                                // positions[i]
                                //           if ( offsets)
        ...
        /** Called once for each run in a line. Can compute baselines and
         * offsets. */
        virtual void
        runInfo(const RunInfo&)
            = 0;
        ...
        /** Called for each run in a line after commitRunInfo. The buffer will
         * be filled out. */
        virtual Buffer
        runBuffer(const RunInfo&)
            = 0;

        /** Called after each runBuffer is filled out. */
        virtual void
        commitRunBuffer(const RunInfo&)
            = 0;
 ...
 private:
    MyShaper(const MyShaper&) = delete;
    MyShaper&
    operator=(const MyShaper&)
        = delete;
};
 ...
 */
class SKSHAPER_API MySkTextBlobBuilderRunHandler final
    : public MyShaper::RunHandler
{
 public:
    MySkTextBlobBuilderRunHandler(const char* utf8Text, SkPoint offset)
