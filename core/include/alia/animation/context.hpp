...
concept animation_context = requires(Context& ctx) {
    ...
    { get_tick_count(ctx) } -> std::same_as<alia_nanosecond_count>;
...
template<animation_context Context>
alia_nanosecond_count
get_animation_ticks_left(Context& ctx, alia_nanosecond_count end_time)
{
    alia_nanosecond_count ticks_remaining = end_time - get_tick_count(ctx);
