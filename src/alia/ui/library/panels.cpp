...
get_panel_style_info(dataless_ui_context& ctx, style_search_path const* path)
{
    ...
    info.border_width = resolve_box_border_width(
        get_layout_traversal(ctx), get_border_width_property(path));

    info.padding = resolve_box_border_width(
        get_layout_traversal(ctx), get_padding_property(path));

    info.border_color = get_color_property(path, "border-color");
    info.background_color = get_color_property(path, "background");
    info.gradient_color = get_property(
        path, "gradient", UNINHERITED_PROPERTY, info.background_color);
    ...
    layout_box const& outer_region)
{
    ...
    if (cache.needs_rendering())
    {
        ...
        resolved_box_corner_sizes border_radii = resolve_box_corner_sizes(
    ...
    interaction_status)
{
    ...
    switch (get_event_category(ctx))
    {
        case RENDER_CATEGORY: {
            ...
            // if (read_signal(style_info).is_rounded
            //     || read_signal(style_info).gradient_color
            //            != read_signal(style_info).background_color)
            {
                // caching_renderer cache(
                //     ctx, data.rendering, style_info.id(), outer_region);
                // if (cache.needs_rendering())
                // {
                //     skia_renderer renderer(
                //         ctx, cache.image(), outer_region.size);
                ...
                // set_color(paint, read_signal(style_info).background_color);
                // draw_rect(
                //     event.canvas,
                //     paint,
                //     background_region,
                ...
                //         border_radii,
                //         read_signal(style_info).border_width));
            ...
            // {
            //     draw_panel_focus_border(
            //         ctx, data.focus_rendering, style_info, outer_region);
            // }
        ...
        case INPUT_CATEGORY:
            ...
            //     && ctx.event->type == MOUSE_PRESS_EVENT
...
panel::outer_region() const
{
    ...
    return remove_border(
        region, as_layout_size(get(data_->style_info).margin));
}

layout_box
panel::padded_region() const
{
    ...
    optional_storage<layout_vector> const& scroll_position_storage)
{
    ...
    if (begin_outer_panel(
            ...
            data->panel,
            make_custom_getter(
                &get(data->style_info), ref(&data->style_info.key.get())),
            outer_,
            layout_spec,
            flags | PANEL_IGNORE_STYLE_PADDING,
            id,
            WIDGET_NORMAL))
    {
        if (!(flags & PANEL_UNSAFE_CLICK_DETECTION))
