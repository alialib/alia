...
void tree_node::begin(context& ctx, layout const& layout_spec, flag_set flags,
    accessor<bool> const* expanded_accessor, region_id expander_id)
{
    ...
    if ((flags & NO_INDENT) == 0)
    {
        ...
        expander_result_ = do_node_expander(ctx, *expanded_accessor,
...
void tristate_tree_node::begin(context& ctx, layout const& layout_spec,
    flag_set flags, accessor<int> const* expanded_accessor,
...
void tristate_tree_node::end_header()
{
    label_region_.end();
}
