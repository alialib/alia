#ifndef ALIA_FLOW_TIMING_HPP
#define ALIA_FLOW_TIMING_HPP
...
void
request_refresh(dataless_ui_context& ctx, ui_time_type duration);

// Request that the UI context refresh again quickly enough for smooth
// animation.
void
request_animation_refresh(dataless_ui_context& ctx);
...
ui_time_type
get_animation_tick_count(dataless_ui_context& ctx);
...
static inline int
interpolate(int a, int b, double factor)
{
    return int(std::floor(a * (1 - factor) + b * factor + 0.5));
}

static inline uint8_t
interpolate(uint8_t a, uint8_t b, double factor)
{
...
interpolate(size_t a, size_t b, double factor)
{
    return size_t(
        std::floor(double(a) * (1 - factor) + double(b) * factor + 0.5));
}
...
// UI events.
void
start_timer(dataless_ui_context& ctx, widget_id id, unsigned duration);
bool
detect_timer_event(dataless_ui_context& ctx, widget_id id);
...
void
restart_timer(dataless_ui_context& ctx, widget_id id, unsigned duration);
