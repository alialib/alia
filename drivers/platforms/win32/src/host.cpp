...
{
    // Overlapped window: frame, title bar, keyboard/mouse, DComp target.
    // Client pixels come from a composition swapchain (no GDI redirection
    // bitmap), presented in sync with WM_NCCALCSIZE during live resize.
    HWND hwnd = nullptr;
    ...
    // True when the adapter supports DXGI_FEATURE_PRESENT_ALLOW_TEARING.
    bool allow_tearing = false;
    // Full backbuffer allocation (matches drawable size for composition).
    UINT buffer_width = 0;
    UINT buffer_height = 0;
    // Active present/source size and UI drawable size.
    UINT width = 0;
    UINT height = 0;
    UINT client_width = 0;
    ...
    // Live-resize: WM_NCCALCSIZE resizes the composition swapchain and
    // presents before DWM commits the new frame. A USER timer keeps
    // animations alive during pure moves / paused resizes (do not nest a
    // message pump inside WM_ENTERSIZEMOVE — that fights the OS modal loop).
...
host_size_move_ensure_timer(alia_win32_host* host)
{
    if (!host->hwnd || host->size_move_timer_armed)
        return;
    // 1ms with timeBeginPeriod(1): wake often; the frame-latency waitable
    // paces presents. Must return from WM_ENTERSIZEMOVE so the OS modal
    // loop can keep moving the window — never nest a message pump there.
    SetTimer(host->hwnd, k_size_move_timer_id, 1, nullptr);
...
UINT
host_swapchain_flags(alia_win32_host const* host)
{
    UINT flags = DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT;
    if (host && host->allow_tearing)
        flags |= DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING;
    return flags;
}

bool
host_query_allow_tearing(IDXGIFactory2* factory)
{
    IDXGIFactory5* factory5 = nullptr;
    if (FAILED(factory->QueryInterface(IID_PPV_ARGS(&factory5))) || !factory5)
        return false;
    BOOL allow = FALSE;
    HRESULT const hr = factory5->CheckFeatureSupport(
...
void
host_release_dcomp(alia_win32_host* host)
{
    if (host->dcomp_visual)
    {
        host->dcomp_visual->SetContent(nullptr);
        host->dcomp_visual->Release();
        host->dcomp_visual = nullptr;
    }
    if (host->dcomp_target)
    {
        host->dcomp_target->Release();
        host->dcomp_target = nullptr;
    }
    if (host->dcomp)
    {
        host->dcomp->Commit();
...
bool
host_bind_dcomp(alia_win32_host* host)
{
    if (!host || !host->hwnd || !host->swapchain)
        return false;
    ...
    if (!host->dcomp)
    {
        ...
        if (FAILED(hr) || !host->dcomp)
        {
            std::fprintf(
                stderr,
                "[alia win32] DCompositionCreateDevice failed (0x%08lx)\n",
                hr);
            return false;
        }

        hr = host->dcomp->CreateTargetForHwnd(
            host->hwnd, FALSE, &host->dcomp_target);
        if (FAILED(hr) || !host->dcomp_target)
        {
            std::fprintf(
                stderr,
                "[alia win32] CreateTargetForHwnd failed (0x%08lx)\n",
                hr);
            host_release_dcomp(host);
            return false;
        }

        hr = host->dcomp->CreateVisual(&host->dcomp_visual);
        if (FAILED(hr) || !host->dcomp_visual)
        {
            std::fprintf(
                stderr, "[alia win32] CreateVisual failed (0x%08lx)\n", hr);
            host_release_dcomp(host);
            return false;
        }

        hr = host->dcomp_target->SetRoot(host->dcomp_visual);
        if (FAILED(hr))
        {
            std::fprintf(
                stderr, "[alia win32] SetRoot failed (0x%08lx)\n", hr);
            host_release_dcomp(host);
            return false;
        }
    }
...
host_create_swapchain(alia_win32_host* host, UINT width, UINT height)
{
    ...
    host->allow_tearing = host_query_allow_tearing(factory);
    ...
    // Composition swapchains require STRETCH. Buffers track the client size
    // exactly, so this does not visually stretch content.
    desc.Scaling = DXGI_SCALING_STRETCH;
    desc.AlphaMode = DXGI_ALPHA_MODE_IGNORE;
    desc.Flags = host_swapchain_flags(host);

    hr = factory->CreateSwapChainForComposition(
        host->device, &desc, nullptr, &host->swapchain);
    factory->Release();
    if (FAILED(hr))
    {
        std::fprintf(
            stderr,
            "[alia win32] CreateSwapChainForComposition failed (0x%08lx)\n",
            hr);
        return false;
    }
...
// Resize the composition swapchain to an exact client size and update the
// drawable metrics. Call with the upcoming client size from WM_NCCALCSIZE
// (GetClientRect is still stale there).
bool
host_sync_swapchain_to_size(alia_win32_host* host, UINT width, UINT height)
{
    ...
    {
        if (!host_create_swapchain(host, width, height))
            return false;
        return true;
    }

    if (host->buffer_width != width || host->buffer_height != height)
    {
        if (!host_resize_buffers(host, width, height))
        {
            if (host->dcomp_visual)
                host->dcomp_visual->SetContent(nullptr);
            if (host->dcomp)
...
host_sync_swapchain_to_client(alia_win32_host* host, bool /*commit_buffers*/)
{
    ...
    return host_sync_swapchain_to_size(host, client_w, client_h);
}
...
host_create_device_and_swapchain(alia_win32_host* host)
{
    ...
    std::fprintf(
        stderr,
        "[alia win32] D3D11 feature level 0x%x, %ux%u, dcomp swapchain ok"
        "%s\n",
        unsigned(got),
        host->width,
        host->height,
        host->allow_tearing ? ", allow_tearing" : "");
    return true;
}
...
// Present one frame for the normal run loop and the size/move timer pump.
// When `nonblocking` is true, returns false if the frame-latency slot is not
// free yet. Live resize presents go through host_present_frame_at_size
// instead (unpaced + tearing).
bool
host_present_frame(alia_win32_host* host, bool nonblocking)
{
    ...
    // Idle vsync and the size/move timer both wait on the latency object so
    // animation stays tied to flip cadence while the modal loop owns the
    // thread (title-bar moves, paused resizes).
    ...
    // Present(1) when idle with vsync. Timer presents during size/move use
    // Present(0) without tearing so they stay paced by the waitable above.
    UINT const sync_interval = (host->vsync && !host->in_size_move) ? 1u : 0u;
    host->swapchain->Present(sync_interval, 0);
    ++host->frame_index;
...
// Present at an explicit upcoming client size (WM_NCCALCSIZE). GetClientRect
// is still stale in that handler, so sync must not read it. Unpaced +
    ...
    alia_win32_host* host, UINT width, UINT height)
{
    ...
    if (!host_sync_swapchain_to_size(host, width, height) || !host->rtv)
    {
        host->presenting = false;
        return false;
    }
    ...
    if (host->allow_tearing && !host->fullscreen)
        present_flags |= DXGI_PRESENT_ALLOW_TEARING;
    host->swapchain->Present(0, present_flags);
    ++host->frame_index;
...
host_wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        ...
        case WM_NCCALCSIZE: {
            // Let DefWindowProc compute the upcoming client rect, then resize
            // the composition swapchain and Present before DWM commits the
            // frame. This is what keeps content in lockstep with the chrome.
            DefWindowProcW(hwnd, msg, wParam, lParam);
            if (host && host->device && host->swapchain)
            {
                RECT const* rect = reinterpret_cast<RECT*>(lParam);
                if (rect && rect->right > rect->left
                    && rect->bottom > rect->top)
                {
                    UINT const w = UINT(rect->right - rect->left);
                    UINT const h = UINT(rect->bottom - rect->top);
                    host_note_size_event(host);
                    host_present_frame_at_size(host, w, h);
                }
            }
            // Never preserve the old client area.
            return 0;
        }
        case WM_ENTERSIZEMOVE:
            if (host)
            {
                ...
                host_note_size_event(host);
                // Return immediately so the OS size/move modal loop can run.
                // Drive animation from a timer; live resize presents from
                // WM_NCCALCSIZE.
        ...
        case WM_TIMER:
            ...
            {
                // Nonblocking: if the flip slot is busy, skip this tick so we
                // do not stall the OS modal loop inside WaitForSingleObject.
                if (host->in_size_move)
                    host_present_frame(host, /*nonblocking=*/true);
                return 0;
            }
            break;
        case WM_SIZE:
            // Live resize presents from WM_NCCALCSIZE. Outside size/move,
            // keep buffers matched after maximize/restore/etc.
            ...
            return 0;
        case WM_ERASEBKGND:
            // DComp owns the client pixels.
            return 1;
        case WM_MOUSEMOVE: {
            alia_ui_system* ui
                = host && host->binding.ui ? host->binding.ui : nullptr;
            if (ui)
            {
                if (!host->tracking_mouse_leave)
                {
                    TRACKMOUSEEVENT tme{};
                ...
                alia_win32_enqueue_mouse_motion(
                    ui,
        ...
        case WM_MOUSELEAVE:
            ...
            if (host && host->binding.ui)
                alia_ui_enqueue_mouse_loss(host->binding.ui);
            return 0;
        case WM_LBUTTONDOWN:
            if (host && host->binding.ui)
            {
        ...
        case WM_DESTROY:
            if (host)
            {
                host_size_move_kill_timer(host);
                host_size_move_restore_timer_period(host);
                // Tear down DComp before the HWND dies so the fade is clean.
                host_release_dcomp(host);
                host->running = false;
            }
            PostQuitMessage(0);
...
alia_win32_host_sync_surface(alia_win32_host* host, alia_ui_system* ui)
{
    ...
    // Prefer the active drawable size tracked by the composition swapchain.
