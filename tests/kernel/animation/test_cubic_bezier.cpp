#include <alia/kernel/animation/cubic_bezier.hpp>

#include <doctest/doctest.h>

using namespace alia;

TEST_CASE("linear cubic bezier")
{
    ...
    for (int i = 0; i <= 10; ++i)
    {
        REQUIRE(
            ...
            == doctest::Approx(solve_for_t_at_x_with_bisection_search(
                                   coeff, 0.1 * i, error_tolerance))
                   .epsilon(error_tolerance * 2));
        REQUIRE(
            eval_curve_at_x(curve, 0.1 * i, error_tolerance)
            == doctest::Approx(0.1 * i).epsilon(error_tolerance));
    }
}

TEST_CASE("nonlinear cubic bezier")
{
    ...
    for (int i = 0; i != 11; ++i)
    {
        ...
        REQUIRE(
            ...
            == doctest::Approx(test_values[i][1]).epsilon(error_tolerance));
    }
}

TEST_CASE("degenerate cubic bezier")
{
