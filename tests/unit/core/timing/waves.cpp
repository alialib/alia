...
TEST_CASE("square_wave", "[timing][waves]")
{
    ...
    std::function<value_signal<bool>(core_context)> square_wave_invoker
        = [&](core_context ctx) { return square_wave(ctx, 50); };

    // Define a utility function for processing timing events.
    auto process_timing_events = [&]() {
        sys.controller = [&](core_context ctx) { square_wave_invoker(ctx); };
    ...
    square_wave_invoker
        = [&](core_context ctx) { return square_wave(ctx, 50, 10); };
    ...
    square_wave_invoker = [&](core_context ctx) {
    ...
    square_wave_invoker
        = [&](core_context ctx) { return square_wave(ctx, 20); };
