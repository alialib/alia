...
function(alia_add_host_library)
    if(TARGET alia_host)
        return()
    endif()
    if(EMSCRIPTEN)
        if(NOT TARGET alia_web_platform)
            message(FATAL_ERROR "alia_host requires alia_web_platform on web")
        endif()
    else()
        if(NOT TARGET alia_glfw_platform AND NOT TARGET alia_win32_platform)
            message(FATAL_ERROR
                "alia_host requires alia_glfw_platform or alia_win32_platform")
        endif()
    endif()

    add_library(alia_host STATIC
        ${CMAKE_SOURCE_DIR}/drivers/platforms/api/src/host.cpp)
    target_include_directories(alia_host PUBLIC
        ${CMAKE_SOURCE_DIR}/drivers/platforms/api/include)
    target_link_libraries(alia_host PUBLIC alia_platform_api)
    if(EMSCRIPTEN)
        target_link_libraries(alia_host PUBLIC alia_web_platform)
    else()
        if(TARGET alia_glfw_platform)
            target_link_libraries(alia_host PUBLIC alia_glfw_platform)
        endif()
        if(TARGET alia_win32_platform)
            target_link_libraries(alia_host PUBLIC alia_win32_platform)
        endif()
    endif()
endfunction()
